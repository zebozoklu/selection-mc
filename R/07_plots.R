# Plots
# Purpose: generate figures for bias/RMSE/sign-fail across scenarios
...
make_plots <- function(summary_path = "output/results/mc_summary.rds") {
  ...
  p_sign <- ggplot(tab_beta1,
                   aes(x = rho_f, y = sign_fail, colour = est_lab, group = est_lab)) +
    geom_line(position = position_dodge(width = 0.2)) +
    geom_point(position = position_dodge(width = 0.2)) +
    facet_grid(err_lab ~ p_label) +
    labs(
      x = expression(rho),
      y = "Sign failure rate for beta[1]",
      colour = "Estimator",
  ...
  p_bias <- ggplot(tab_beta1,
                   aes(x = rho_f, y = bias, colour = est_lab, group = est_lab)) +
  ...
  invisible(list(sign_plot = p_sign, bias_plot = p_bias))
}
