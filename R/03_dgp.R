...
# Outputs: list(Y, S, X, Z, y_star, s_star, p_hat, uv_corr)
# Sanity checks: mean(S) near target; dimensions consistent; no NA explosions

# --- helper: build SigmaX (AR correlation for 2 regressors) ---
...
# We want marginals ~ logistic with mean 0 and variance 1, and dependence controlled by rho.
# Construction:
#   1) draw bivariate normal with corr = rho
#   2) map to U(0,1) via Phi
#   3) map to logistic via qlogis (logistic(0,1) has var = pi^2/3)
#   4) rescale to variance 1 using sqrt(3)/pi
draw_uv_logistic <- function(n, rho) {
  ...
  Lraw <- qlogis(U)               # logistic(0,1)
  scale <- sqrt(3) / pi           # make var = 1
  Lstd <- Lraw * scale
...
simulate_one_dataset <- function(scen, cfg, SigmaX = NULL) {
  ...
  # --- unpack basic stuff ---
  n    <- as.integer(scen$n)
  rho  <- as.numeric(scen$rho)
  beta <- cfg$beta
  gamma_slopes <- cfg$gamma_slopes
  
  # --- SigmaX ---
  if (is.null(SigmaX)) {
    # use config default if available; fallback to 0.3
    ar <- if (!is.null(cfg$SigmaX_ar)) cfg$SigmaX_ar else 0.3
    SigmaX <- make_SigmaX(ar = ar)
  }
  ...
  errs <- switch(
    ...
    "t3"       = draw_uv_t(n, rho, df = as.integer(scen$df)),
    "t5"       = draw_uv_t(n, rho, df = as.integer(scen$df)),
  ...
  # diagnostics
  p_hat  <- mean(S)
  uv_corr <- suppressWarnings(cor(u, v))
